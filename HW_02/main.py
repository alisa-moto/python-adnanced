...
if __name__ == '__main__':
    ...
    def execute_command(command_key):
        # lib = Library(books, readers)
        reader = Reader()
        if command_key == 1: lib.add_book_to_library()
        if command_key == 2: lib.delete_book_from_library()
        if command_key == 3: lib.give_book_to_reader(*Library.ask_for_ids())
        if command_key == 4: lib.take_book_from_reader(*Library.ask_for_ids())
        if command_key == 5: lib.print_all_books()
        if command_key == 6: lib.print_books_in_library()
        if command_key == 7: lib.print_taken_books()
        if command_key == 8: lib.sort_books_by_name()
        if command_key == 9: lib.sort_books_by_author()
        if command_key == 10: lib.sort_books_by_date()
        if command_key == 11: lib.delete_reader()
        if command_key == 12: lib.print_all_readers()
        if command_key == 13: lib.print_readers_with_book()
        if command_key == 14: lib.readers_list.append(reader.create_reader())
        if command_key == 15: lib.give_book_to_reader(*Reader.reader_take_or_give_book())
        if command_key == 16: lib.take_book_from_reader(*Reader.reader_take_or_give_book())
